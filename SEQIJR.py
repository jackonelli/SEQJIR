...
class SEQIJR:
    ...
    def derivative(self, y):
        ...
        Sp = self.Pi - self.b * y[0] * (
            y[3] + self.e_E * y[1] + self.e_Q * y[2] +
            self.e_J * y[4]) / self.N - self.mu * y[0]
        Ep = self.b * y[0] * (y[3] + self.e_E * y[1] + self.e_Q * y[2] +
                              self.e_J * y[4]) / self.N - (
                                  self.g_1 + self.k_1 + self.mu) * y[1]
        Qp = self.g_1 * y[1] - (self.k_2 + self.mu) * y[2]
        Ip = self.k_1 * y[1] - (self.g_2 + self.d_1 + self.s_1 +
                                self.mu) * y[3]
        Jp = self.g_2 * y[3] + self.k_2 * y[2] - (self.s_2 + self.d_2 +
                                                  self.mu) * y[4]
        ...
        return np.array([Sp, Ep, Qp, Ip, Jp, Rp, aJp, aIJp, aDp],
    ...
    def R_c(self):
        ...
        if (D_1 == 0) or (D_2 == 0) or (D_3 == 0) or (D_4 == 0):
            ...
            R_c = self.b * (self.e_E / D_1 + self.k_1 /
                            (D_1 * D_2) + self.e_Q * self.g_1 /
                            (D_1 * D_4) + self.e_J * self.k_1 * self.g_2 /
                            (D_1 * D_2 * D_3) +
